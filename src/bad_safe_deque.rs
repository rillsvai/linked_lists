...
impl<T> DoublyLinkedList<T> {
    ...
    fn peek(link: &Link<T>) -> Option<Ref<T>> {
        link.as_ref()
            .map(|node| Ref::map(node.borrow(), |node| &node.elem))
    }

    fn peek_mut(link: &Link<T>) -> Option<RefMut<T>> {
        link.as_ref()
...
#[cfg(test)]
mod tests {
    use crate::bad_safe_deque::DoublyLinkedList;
